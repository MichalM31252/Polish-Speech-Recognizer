...
if_spectrogram = False
if_charts_of_sentences = True
...
print("author_directories = ", author_directories)
print("number of authors = ", len(author_directories))
...
for aut_dir in author_directories:
    ...
    for sentence_name in sentence_names:
        ...
        samplerate, sentence_data = wavfile.read(wav_fname)
        #pdb.set_trace()
        #print(f"number of channels = {data.shape[1]}")
        num_of_samples = sentence_data.shape[0]

        length = num_of_samples / samplerate
        print(f"length = {length}s, samplerate = {samplerate}1/s")

        sd.play(sentence_data,samplerate=samplerate)

        if if_charts_of_sentences:
            if if_chart_in_the_time_domain:
                time = np.linspace(0., length, sentence_data.shape[0])
                #plt.plot(time, data[:, 0], label="Left channel")
                #plt.plot(time, data[:, 1], label="Right channel")
                plt.plot(time, sentence_data, label="channel")
                plt.title("sentence: " + aut_path + "/" + sentence_name )
                plt.legend()
                plt.xlabel("Time [s]")
                plt.ylabel("Amplitude")
                plt.show()
            if if_spectrogram:
                plt.specgram(sentence_data)
                plt.title("sentence: " + aut_path + "/" + sentence_name )
                plt.legend()
                plt.xlabel("Time s")
            ...
            for i in range(len(grid_lines)-grid_line_index):
                ...
                if len(tokens) >= 3:
                    ...
                    if (tokens[1] == "=")&(tokens[2][1:-1] == word):
                        ...
                        word_data = sentence_data[sample_no_start:sample_no_end]
                        ...
                        if if_spectrogram:
                            spectrogram = plt.specgram(word_data)
                            plt.title(aut_path + "/" + sentence_name + "   word: " + word)
                            plt.legend()
                            plt.xlabel("Time ")
                            plt.ylabel("Frequency")
